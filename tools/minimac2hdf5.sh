...
zcat $id_file | gawk '{ print $1}' | gawk 'BEGIN{FS="->"}{if(NF==2){print $2}else{print $1}}' >> ${SAVE_DIR}/${SUBJECT_ID_FILE}
zcat $id_file | gawk '{ print $1}' | gawk 'BEGIN{FS="->"}{if(NF==2){print $1}else{print "NO FAM DATA";exit}}' >> ${SAVE_DIR}/${SUBJECT_FAM_FILE}
